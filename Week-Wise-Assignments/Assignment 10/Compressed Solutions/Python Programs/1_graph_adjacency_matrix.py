class GraphMatrix:
    ...
    def add_edge(self, src, dest):
        if src < self.vertices and dest < self.vertices:
            self.matrix[src][dest] = self.matrix[dest][src] = 1
    
    def count_vertices(self): return self.vertices
    
    def count_edges(self):
        return sum(self.matrix[i][j] for i in range(self.vertices) for j in range(i, self.vertices))
    
    def find_adjacent(self, v):
        return [i for i in range(self.vertices) if self.matrix[v][i]]
